...
const StorePage = () => {
  ...
  useEffect(() => {
    const loadStoreData = async () => {
      try {
        ...
        // If no storeId provided, use default store (Brilliant Channel)
        const targetStoreId = storeId || 'brilliant-channel-001';

        // Load store data
        const storeData = await dataService.getStoreById(targetStoreId);
        setStore(storeData);

        // Load products for this store
        const productsResponse = await dataService.getProductsByStore(targetStoreId, {
          limit: 50, // Load more products for better demo
          sortBy: 'featured'
        });
  ...
  const filteredProducts = products.filter(product => {
    ...
    return product.category.name === selectedCategory || product.category.id === selectedCategory;
  }).sort((a, b) => {
    switch (sortBy) {
      case 'price-low': return a.price.current - b.price.current;
      case 'price-high': return b.price.current - a.price.current;
      case 'rating': return b.rating.average - a.rating.average;
      case 'newest': return new Date(b.createdAt) - new Date(a.createdAt);
      case 'sold': return b.sales.totalSold - a.sales.totalSold;
                ...
                <div className="flex-1 text-white">
                  ...
                  <div className="flex flex-wrap gap-6 text-sm">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="opacity-75">({store.reviewsCount} reviews)</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>{store.location.city}, {store.location.province}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {store.categories.map(category => {
                    const count = products.filter(p => p.category.name === category || p.category.id === category).length;
                    return (
                      <button
                        key={category}
                      ...
                      {product.stock.inStock && (
                        <div 
                          className={`text-xs px-2 py-1 rounded-full inline-block ${
                            product.stock.quantity <= 5 ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'
                          }`}
                        >
                          {product.stock.quantity <= 5 ? `Only ${product.stock.quantity} left` : 'In Stock'}
                        </div>
                      )}
                      {!product.stock.inStock && (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2 text-sm" style={{ color: theme.colors.gray[600] }}>
                  <div>🚚 {store.policies.shipping}</div>
                  <div>↩️ {store.policies.returns}</div>
                  <div>🛡️ {store.policies.warranty}</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm" style={{ color: theme.colors.gray[600] }}>
                  <div>📈 Response Rate: {store.responseRate}</div>
                  <div>⏱️ Response Time: {store.responseTime}</div>
                </div>
              </div>
